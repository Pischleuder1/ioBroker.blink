...
class MjpegServer {
	...
	_serveMjpeg(req, res, entry) {
		...
		this.adapter.log.debug(`MJPEG: client connected for ${entry.meta.name} (${entry.clients.size} aktiv)`);
		...
		const cleanup = () => {
			...
			entry.lastClientTs = Date.now();
			this.adapter.log.debug(`MJPEG: client disconnected for ${entry.meta.name} (${entry.clients.size} aktiv)`);
		};
