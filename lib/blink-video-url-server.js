...
if (typeof globalThis.__blinkServer !== 'undefined') {
	try {
		...
		log('Vorherigen Blink-Server gestoppt');
	} catch (e) {
		/* ignore */
	}
}
...
function readBlinkCredentials() {
	return new Promise((resolve, reject) => {
		...
		function finish(source, creds) {
			log(
				`Blink Config gelesen aus ${source}: email=${creds.email ? 'ja' : 'nein'}, password=${
					creds.password ? 'ja' : 'nein'
			...
			if (!creds.email || !creds.password) {
				reject(
					new Error(
						`Blink E-Mail oder Passwort fehlt in system.adapter.${BLINK_INSTANCE}.native.email/password`,
					),
				);
				return;
			}
		...
		function tryCliFallback(reason) {
			...
			exec(cmd, { timeout: 15000 }, (err, stdout, stderr) => {
				if (err) {
					reject(
						new Error(
							`Blink Adapter-Konfiguration konnte nicht gelesen werden. getObject: ${reason} / CLI: ${
...
function detectLfrLiveViewRejection(session) {
	...
	if (isOldLfrPath && lfrAckZero) {
		...
		return {
			reason:
				`XT2/LFR LiveView wurde von Blink nicht bestaetigt: lfr_ack=0, stage_lv=null${hasCommandErrorText}. ` +
				`Diese Kamera liefert ueber den aktuellen immis/HLS-Weg keinen nutzbaren Stream.`,
			detail: {
...
async function markCameraUnsupported(cameraId, reason) {
	...
	if (!objectExists(id)) {
		log(`markCameraUnsupported: State ${id} existiert nicht – Adapter neu starten?`, 'warn');
		return;
	}
	try {
		...
		if (ok) {
			log(`Kamera ${cameraId} dauerhaft als "kein LiveView" markiert. Grund: ${reason || 'unbekannt'}`, 'warn');
		}
	} catch (e) {
		log(`markCameraUnsupported fuer ${cameraId} fehlgeschlagen: ${e.message || e}`, 'warn');
	}
}
...
async function buildLiveviewConfigForCamera(cam) {
	...
	if (!accountId && accountFallback.ambiguous) {
		warnings.push(
			`Mehrere Account-IDs gefunden (${accountFallback.all.join(
				', ',
			)}), aber keine Kamera-accountId. Bitte accountId per LIVEVIEW_CAMERA_OVERRIDES setzen.`,
		);
	}

	if (!networkId && networkFallback.ambiguous) {
		warnings.push(
			`Mehrere Sync-Module gefunden (${networkFallback.all.join(
				', ',
			)}), aber keine Kamera-networkId. Bitte networkId per LIVEVIEW_CAMERA_OVERRIDES setzen.`,
		);
	}
...
async function discoverCameras() {
	...
	for (const c of cams) {
		...
		// Wenn der Adapter diese Kamera bereits als "LiveView nicht unterstützt"
			...
			try {
				...
				if (st && st.val === true) {
					...
					c.liveMissing = (c.liveMissing || []).concat([
						'Kameramodell unterstützt keinen LiveView (vom Adapter erkannt)',
					]);
				}
			} catch (e) {
	...
	try {
		...
		for (const file of files) {
			...
			try {
				...
				log(`HLS-Datei konnte nicht geloescht werden: ${full} / ${e.message}`, 'warn');
			}
		}
		try {
...
async function startLiveForCamera(cameraId, req) {
	...
	if (!cam) {
		throw new Error(`Unbekannte Kamera-ID: ${cameraId}`);
	}
	if (!cam.liveview) {
		throw new Error(
			`LiveView nicht konfiguriert fuer Kamera ${cameraId}: fehlt ${(cam.liveMissing || []).join(', ')}`,
		);
	}
	...
	if (!session || !session.server || !String(session.server).startsWith('immis://')) {
		...
		await markCameraUnsupported(cameraId, 'kein immis:// Server in Session');
		throw new Error(msg);
	}
	...
	if (!ok) {
		...
		// auch bei Cams auftreten, die LiveView eigentlich können (z. B. Mini, die erst
	...
	// Erfolg: einen evtl. zuvor gesetzten unsupported-Marker zuruecknehmen.
	...
	if (objectExists(unsupportedId)) {
		try {
			...
			if (prev && prev.val === true) {
				...
				log(`Kamera ${cameraId}: unsupported-Marker zurueckgenommen (LiveView funktioniert wieder).`, 'info');
			}
		} catch (e) {
			/* nicht kritisch */
		}
...
function buildCard(cam) {
  ...
  liveBtn.textContent = cam.liveCapable ? '📡 Live' : '🚫 Kein LiveView';
...
function updateLiveButtons() {
  Object.keys(cards).forEach(function(id) {
    ...
    if (c.liveBtn) {
      ...
      if (!c.liveCapable) {
        c.liveBtn.textContent = '🚫 Kein LiveView';
      } else {
        c.liveBtn.textContent = isStarting ? '⏳ Startet…' : (isActive ? '📡 Live läuft' : '📡 Live');
      }
      c.liveBtn.title = !c.liveCapable
        ? ('LiveView nicht verfügbar: ' + (c.liveMissing || []).join(', '))
        : (isActive ? 'LiveView läuft bereits' : 'LiveView starten');
    }

    if (c.stopBtn) {
      c.stopBtn.disabled = !(isStarting || isActive);
      c.stopBtn.textContent = isStarting ? '✕ Abbrechen' : '▪ Stop';
      c.stopBtn.title = isStarting ? 'Start abbrechen/LiveView stoppen' : (isActive ? 'LiveView stoppen' : 'Kein LiveView aktiv');
    }
  ...
  socket.on('stateChange', function(id, state) {
    ...
    for (const cam of list) {
      ...
      if (id === cam.unsupported_datapoint) {
        const UNSUPPORTED_MSG = 'Kameramodell unterstützt keinen LiveView (vom Adapter erkannt)';
	...
	try {
		...
		if (urlPath === '/live/start') {
			...
			if (!cameraId) {
				...
				res.end(JSON.stringify({ ok: false, error: 'camera fehlt' }));
				return;
			}
			...
			if (objectExists(unsupportedStateId)) {
				try {
					...
					if (st && st.val === true) {
						log(
							`LiveView fuer Kamera ${cameraId} blockiert: vom Adapter als nicht unterstuetzt markiert.`,
							'warn',
						);
						res.writeHead(409, {
							'Content-Type': 'application/json; charset=utf-8',
						...
						res.end(
							JSON.stringify({
								ok: false,
								error: 'Kameramodell unterstützt keinen LiveView (vom Adapter erkannt)',
								unsupported: true,
							}),
						);
						return;
