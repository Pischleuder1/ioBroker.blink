...
function normalizeApiHost(apiHost) {
	if (!apiHost) {
		throw new Error('apiHost fehlt. Login-Session enthaelt keine API-Host-Angabe.');
	}
...
/**
 * Startet eine LiveView-Session über die Blink-API.
 *
...
async function getLoginSession() {
	...
	const pin = Object.prototype.hasOwnProperty.call(process.env, 'BLINK_PIN')
		? String(process.env.BLINK_PIN || '').trim()
		: '';
...
async function cli() {
	try {
		...
		if (!token) {
			throw new Error('Login ok, aber kein accessToken/token in Session gefunden.');
		}
		if (!apiHost) {
			throw new Error('Login ok, aber kein apiHost in Session gefunden.');
		}
		if (!accountId) {
			throw new Error('Account-ID fehlt. Setze BLINK_ACCOUNT_ID.');
		}
		if (!networkId) {
			throw new Error('Network-ID fehlt. Setze BLINK_NETWORK_ID.');
		}
		if (!deviceId) {
			throw new Error('Device-ID fehlt. Setze BLINK_DEVICE_ID.');
		}
		...
		try {
			...
			// Schreiben der Fehlerdatei selbst fehlgeschlagen – bewusst ignoriert.
		}
		console.error('\nFEHLER:');
