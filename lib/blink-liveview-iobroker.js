...
const { setTimeout: sleepTimeout } = require('node:timers/promises');
...
function normalizeApiHost(apiHost) {
	if (!apiHost) {
		throw new Error('apiHost is missing. The login session does not contain an API host value.');
	}
...
function readJsonResponse(res, rawBody, method, urlPath) {
	...
	if (contentType.includes('application/json') || /^[\s\r\n]*[[{]/.test(rawBody)) {
		try {
			...
			throw new Error(
				`Antwort war kein gueltiges JSON bei ${method} ${urlPath}: ${err.message}\nBody=${rawBody}`,
			);
		}
	}
	return rawBody;
...
/**
 * Führt einen HTTP(S)-Request gegen die Blink-API aus und parst die JSON-Antwort.
 *
...
/**
 * Baut den LiveView-Start-Pfad abhängig vom Gerätetyp.
 *
 ...
 * @param {string} root0.deviceType - Der Gerätetyp (owl, doorbell, camera).
 * @param {string|number} root0.deviceId - Die Geräte-ID.
...
function liveViewStartPath({ accountId, networkId, deviceType, deviceId }) {
	...
	throw new Error(`Unknown device type: ${deviceType}. Allowed: owl, doorbell, camera`);
}

/**
 * Baut den Poll-Pfad für ein LiveView-Command.
 *
 * @param {object} root0 - Pfad-Parameter.
 * @param {string|number} root0.accountId - Die Account-ID.
 * @param {string|number} root0.networkId - Die Netzwerk-ID.
 ...
 * @param {string} root0.deviceType - Der Gerätetyp.
 * @param {string|number} root0.deviceId - Die Geräte-ID.
 * @param {string|number} root0.networkId - Die Netzwerk-ID.
 * @param {string|number} root0.accountId - Die Account-ID.
 * @param {string} [root0.deviceSerial] - Die Seriennummer des Geräts.
...
/**
 * Starts a LiveView session through the Blink API.
 *
 ...
 * @param {boolean} [root0.stopOnComplete] - Abbruch, sobald complete=true.
...
/**
 * Startet eine LiveView-Session und pollt anschließend den Command-Endpoint.
 *
 * @param {object} options - Optionen für Start und Poll (siehe startLiveView und pollLiveView).
...
async function getLoginSession() {
	...
	const email = env('BLINK_EMAIL') || (await question('Blink email'));
	const password = env('BLINK_PASSWORD') || (await question('Blink password', { silent: true }));
	const pin = Object.prototype.hasOwnProperty.call(ENV, 'BLINK_PIN') ? String(ENV.BLINK_PIN || '').trim() : '';
...
async function cli() {
	try {
		...
		if (!token) {
			throw new Error('Login succeeded, but no accessToken/token was found in the session.');
		}
		if (!apiHost) {
			throw new Error('Login succeeded, but no apiHost was found in the session.');
		}
		if (!accountId) {
			throw new Error('Account ID missing. Set BLINK_ACCOUNT_ID.');
		}
		if (!networkId) {
			throw new Error('Network ID missing. Set BLINK_NETWORK_ID.');
		}
		if (!deviceId) {
			throw new Error('Device ID missing. Set BLINK_DEVICE_ID.');
		}

		console.error(`Login succeeded. API host: ${apiHost}`);
